...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "lambda", ScmPair (ScmNil, exprs)) ->
      let expr = tag_parse (ScmPair(ScmSymbol "begin", exprs)) in
      ScmLambda([], Simple, expr)
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
       let params = (match (scheme_list_to_ocaml ribs) with
       | (sexprs', ScmNil) -> (List.map (fun x -> match x with
          | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) -> ScmSymbol var
          | _ -> raise (X_syntax "invalid let rib")) sexprs')
       | _ -> raise (X_syntax "invalid let ribs")) in
       let args = (match (scheme_list_to_ocaml ribs) with
       | (sexprs', ScmNil) -> (List.map (fun x -> match x with
          | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) -> expr
          | _ -> raise (X_syntax "invalid let rib")) sexprs') 
       | _ -> raise (X_syntax "invalid let ribs")) in
      let lambda = ScmPair (ScmSymbol "lambda",
                            ScmPair (scheme_sexpr_list_of_sexpr_list params, exprs)) in
      tag_parse (ScmPair (lambda, scheme_sexpr_list_of_sexpr_list args))
                ...
                (ScmPair 
                    (ScmPair (var, ScmPair (arg, ScmNil)),
                      ribs),
                        exprs)) -> 
                          tag_parse (ScmPair (ScmSymbol "let",
                                      ScmPair (ScmPair (ScmPair (var, ScmPair (arg, ScmNil)), ScmNil),
                                        ScmPair (ScmPair (ScmSymbol "let*", ScmPair (ribs, exprs)), ScmNil))))
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       let newRibs = (match (scheme_list_to_ocaml ribs) with
        | (sexprs', ScmNil) -> (List.map (fun x -> match x with
            | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) -> ScmPair(ScmSymbol var, ScmPair(ScmSymbol "'whatever", ScmNil))
            | _ -> raise (X_syntax "invalid let rib")) sexprs')
        | _ -> raise (X_syntax "invalid let ribs")) in
       let argsSet = (match (scheme_list_to_ocaml ribs) with
        | (sexprs', ScmNil) -> (List.map (fun x -> match x with
            | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) -> ScmPair (ScmSymbol "set!", ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)))
            | _ -> raise (X_syntax "invalid letrec rib")) sexprs') 
        | _ -> raise (X_syntax "invalid letrec ribs")) in
      let exprsList = (match (scheme_list_to_ocaml exprs) with
        | (sexprs', ScmNil) -> argsSet @ sexprs'
        | _ -> raise (X_syntax "invalid letrec exprs")) in
      tag_parse (ScmPair (ScmSymbol "let", ScmPair (scheme_sexpr_list_of_sexpr_list newRibs, 
                                                    scheme_sexpr_list_of_sexpr_list exprsList)))
    | ScmPair (ScmSymbol "or", ScmNil) ->  ScmConst(ScmBoolean false)
    | ScmPair (ScmSymbol "or", exprs) -> (match (scheme_list_to_ocaml exprs) with
      | exprs', ScmNil -> ScmOr(List.map tag_parse exprs')
      | _ -> raise (X_syntax "invalid or exprs"))
...
module Semantic_Analysis (*: SEMANTIC_ANALYSIS*) = struct 
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function(*func change to support before finish applic for Tail_Call*)
      ...
      | ScmApplic' (proc, args, app_kind) ->
         ...
         then ScmApplic' (run false proc,
                          ...
                          Tail_Call) (*need to be Tail_Call*)
  ...
  (* let should_box_var name expr params = 
    ...
    let pairs = cross_product reads writes in
    (* let (reads, writes) = (List.map (fun (x,y)-> x) reads, List.map (fun (x,y)-> x) writes) in *)
    let find_bond (major,minor) env = List.nth (List.nth env major) minor in
    let get_rib m env = List.nth env m in
    let check_same_rib lex1 lex2 r_env w_env = match (lex1, lex2) with
      | (Bound (i1, j1), Bound (i2, j2)) -> get_rib i1 r_env == get_rib i2 w_env in
    let check_lex lex1 lex2 r_env w_env = match (lex1, lex2) with
      | (Free, Free) -> false
      | (Free, _) -> false
      | (_, Free) -> false
      | (Param i, Param j) -> false
      | (Param i, Bound (i1, j1)) -> true
      | (Bound (i1, j1), Param i) -> true
      | (Bound (i1, j1), Bound (i2, j2)) -> (not (check_same_rib lex1 lex2 r_env w_env)) in  
    let rec loop pairs f = match pairs with
      | [] -> false
      | pair :: pairs' -> if f pair
                            then true
                            else loop pairs' f in
     let check (r, w) = match (r, w) with
      | ((Var' (r_name, r_lex), r_env), (Var' (w_name, w_lex), w_env)) ->
         if (check_lex r_lex w_lex r_env w_env)
         then true
         else false
      | _ -> false in     
    loop pairs check;; *)
  let should_box_var name expr params = 
    ...
    let pairs = cross_product reads writes in
    let get_rib m env = List.nth env m in
    let compareVars ((lx1,env1),(lx2,env2)) = match (lx1,lx2) with
      | (Free, Free) -> true
      | (Free, _) -> true
      | (_, Free) -> true
      |(Param i, Param j) -> true
      |(Param i ,Bound(ma2,mi2))-> false
      |(Bound(ma1,mi1) ,Param i)-> false
      |(Bound(ma1,mi1) ,Bound(ma2,mi2))-> get_rib ma1 env1 == get_rib ma2 env2 in
    let findLexicalAndEnv = fun ((name1,e1),(name2,e2))-> match (name1,name2) with
    | (Var'(v1,l1),Var'(v2,l2)) -> ((l1,e1),(l2,e2)) in
    let getLexicalAndEnv = List.map findLexicalAndEnv pairs in 
    let extract = fun b1 ((lx1,env1),(lx2,env2))-> b1 && (compareVars ((lx1,env1),(lx2,env2))) in
    let result = List.fold_left  extract true getLexicalAndEnv in 
    not result 
    ...
    | ScmLambda' (params, Opt opt, expr') ->
        ...
        let new_body = 
          List.fold_left
            ...
            (auto_box expr')
            box_these in
        let new_sets = make_sets box_these (opt :: params) in
        let new_body = 
          match box_these, new_body with
          | [], _ -> new_body
          | _, ScmSeq' exprs -> ScmSeq' (new_sets @ exprs)
